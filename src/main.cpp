...
static void print_help(const char *progname)
{
	...
	LOG_RAW("  -e, --erase                         Erase and check flash only (no programming)\n");
	LOG_RAW("  -d, --debug_mode                    Enable debug messages\n");
	LOG_RAW("  -p, --dump_register <offset> [len]  Dump register(s) from device (max 256 bytes)\n");
...
int main(int argc, char **argv)
{
	...
	const char *optstring = "f:s:p:uedvh";
	...
	ret = init_file(const_cast<char*>(filename.c_str()));
	if (ret) {
		return ret;
	}
	...
	if (USE_SPI(flags)) {
	    updater = createSPIUpdater();
	} else {
	    updater = createI2CUpdater();
	}
	...
	/* for I2C, ensure chip ready (I2CUpdater overrides wait_for_chip_ready) */
	if (!USE_SPI(flags)) {
		ret = updater->wait_for_chip_ready();
		if (ret) {
